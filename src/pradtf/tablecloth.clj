...
;; both these work so tablecloth issue may be a version one
...
(->> "seattle-weather"
     ...
     (api/dataset-name))
;;TODO doesn't seem to set the ds name since the weblink shows up - just copies it immutably
...
;; * group-by
;;TODO understand grouping
;; *** group
(clojure.pprint/pprint (take 2 (api/rows cw :as-maps)))

(api/group-by DS :V1);; => _unnamed [2 3]:
...
(gDS :data)
;; => #tech.ml.dataset.column<dataset>[2]
:data
[Group: 1 [5 4]:
